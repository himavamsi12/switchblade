  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const lenis = new Lenis({
        ...
        // for miles). 0.6 → 0.35 → 0.5 → 0.8 (by request): even a hard/fast flick should only
        // cover a short distance rather than banking a long glide, but 0.35 read as too heavy.
        // Lower = heavier/slower; 1 = native sensitivity. Reliable and Lenis-native — no input
        // interception, so it can't fight the scene holds or programmatic scrolls.
        wheelMultiplier: 0.8,
      });
